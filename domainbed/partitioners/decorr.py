...
class Decorr(Partitioner):
    ...
    def split(self, dataset):
        ...
        for j in range(0, self.num_environments-1):
            ...
            X_r = torch.tensor(np.array([dataset[env][i] for (env, i) in remaining_indexes])).cuda()
            env_w = (torch.randn(num_remaining).double().cuda() * (1-self.p0) + self.p0).requires_grad_()
    ...
    def _distance_to_identity(self, X_r, w):
        ...
        R = torch.zeros((feature_dimensions, feature_dimensions)).cuda()
        for i in range(feature_dimensions):
            for j in range(feature_dimensions):
                R[i, j] = self._feature_correlation(X_r, i, j, w)
        return torch.linalg.norm( R - torch.eye(feature_dimensions).cuda() )
